...
from src.core.task import Task, TaskBuilder, Trigger
import typing
...
class DailyTaskEdit(QDialog):
    """周期任务编辑组件"""
    task_edited = pyqtSignal(Trigger)
    def __init__(self,wtitle: str, trigger: 'Trigger',
                 *args, **kwargs) -> None:
        super().__init__(*args, **kwargs)
        ...
        self.cycle = QComboBox(self)
        self.cycle.addItems(Trigger.hint.values())
        if self.trigger.cycle:
            self.cycle.setCurrentText(self.trigger.get_trigger_description_str())
        self.cycle.activated[str].connect(self.trig_edit)
        layout.addRow(QLabel("周期性", self), self.cycle)
    ...
    def confirm(self):
        if self.checkForm():
            self.task_edited.emit(self.trigger)
            self.accept()
    def cancel(self):
        self.reject()
    ...
    def checkForm(self) -> bool:
        ...
        if not self.task.category:
            self.task.category = []
        if not self.trigger.cycle:
            self.trigger.cycle = Trigger.PER_DAY
        return True
    def title_edit(self, text: str):
        self.task.title = text
    ...
    def trig_edit(self, text: str):
        t = None
        if text == Trigger.hint[Trigger.ONCE]:
            t = Trigger.ONCE
        if text == Trigger.hint[Trigger.PER_DAY]:
            t = Trigger.PER_DAY
        if text == Trigger.hint[Trigger.PER_WEEK]:
            t = Trigger.PER_WEEK
        if text == Trigger.hint[Trigger.PER_MONTH]:
            t = Trigger.PER_MONTH
        
        self.trigger.cycle = t
